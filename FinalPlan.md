...
### Goal
Make Anthropic Claude the primary reasoning engine for ArgusV2. Integrate a lightweight provider abstraction so Gemini remains functional as a secondary option, but default everything — config, demo, submission, docs — to Anthropic. Win the Anthropic Grand Prize.
...
### Strategy
**Anthropic-first, Gemini-available.** Build a thin provider abstraction (`LLMClient` protocol with `generate(contents: str) → str`), default everything to Anthropic, keep Gemini as a secondary backend. Demo, submission, and docs all lead with Claude. The abstraction costs ~60 lines and enables "we also support Gemini" as an enterprise talking point without diluting the Anthropic prize pitch.
...
#### Step 3.1 — Create `LLMClient` provider abstraction
**New file**: `src/core/llm_provider.py`

Define a minimal provider interface and two concrete backends:

```python
# Protocol / base
class LLMClient:
    provider_name: str   # "anthropic" or "gemini" — recorded in traces
...
def create_llm_client(provider: str, model: str | None = None) -> LLMClient:
    ...
    # Missing API key or SDK → raise ConfigurationError (fail-closed at startup, not at call time)
```
...
**Acceptance**: `create_llm_client("anthropic")` returns a working client when `ANTHROPIC_API_KEY` is set. `create_llm_client("gemini")` returns a working client when `GEMINI_API_KEY` is set. Missing key raises `ConfigurationError`.
...
**Acceptance**: `python -m src.adapters.cli --provider anthropic` and `python -m src.adapters.cli --provider gemini` both work. Default is Anthropic.
...
#### Step 3.4 — Add provider provenance to traces and reports
**Files**: `src/core/pipeline.py` (`_write_manifest`, `_write_summary`), `src/core/reporter.py` (`render_json_report`, `render_mr_comment`)
...
**Acceptance**: After a run, `grep -r "anthropic" .argus-trace/` returns hits in manifest.json. `argus_report.json` contains `"provider": "anthropic"`. MR comment shows provider.
...
- `test_missing_api_key_raises` — `create_llm_client("anthropic")` without `ANTHROPIC_API_KEY` raises `ConfigurationError`
...
**Acceptance**: Full test suite passes (`pytest tests/`). No test references `genai.Client` directly.

#### Step 3.7 — Update agent/flow configs and docs
**Files**: `config.yml`, `.gitlab/duo/agent-config.yml`, `docs/submission-text.md`

`config.yml`:
- Update `description` to mention "Anthropic Claude" as the reasoning engine
...
#### Step 3.8 — Validate end-to-end Anthropic mode
**Validation run** (manual or CI):

1. Set `ANTHROPIC_API_KEY` env var
2. Run benchmark corpus: `python -m src.adapters.cli --repo-path . --mode ci --provider anthropic --allow-local-verify`
3. Verify outcomes:
   - `saturating_withdrawal.py` (safe) → VERIFIED
   - `negative_withdrawal.py` (vulnerable) → VULNERABLE
   - Verdicts match Phase 1 baseline (same deterministic core, different LLM)
4. Inspect artifacts:
   - `manifest.json` shows `"provider": "anthropic"`
...
**Acceptance**: Anthropic mode produces valid end-to-end results on the seeded benchmark corpus. Gemini mode still works.

### Deliverables summary
| # | Deliverable | File(s) | New/Modified |
|---|---|---|---|
| 3.1 | Provider abstraction | `src/core/llm_provider.py` | New |
...
| 3.4 | Provenance in traces/reports | `src/core/pipeline.py`, `src/core/reporter.py` | Modified |
| 3.5 | Anthropic SDK dependency | `requirements.txt` | Modified |
| 3.6 | Test updates | `tests/test_llm_provider.py`, `tests/test_repair.py`, `tests/test_llm_translator.py` | New + Modified |
| 3.7 | Config/doc updates | `config.yml`, `agent-config.yml`, `docs/submission-text.md` | Modified |
| 3.8 | End-to-end validation | (manual/CI run) | N/A |

### Acceptance criteria
- [ ] `--provider anthropic` with `ANTHROPIC_API_KEY` drives full discover → translate → verify → proof_search → repair flow
- [ ] Default provider is `"anthropic"` everywhere (config, CLI, docs)
...
- [ ] Trace manifests contain `"provider"` and `"model"` fields
- [ ] JSON report, MR comment, and SARIF report show provider attribution
- [ ] All existing tests pass with updated mocks + new provider tests pass
- [ ] Anthropic mode produces correct verdicts on seeded benchmark corpus (VERIFIED for safe, VULNERABLE for vulnerable)
- [ ] Deterministic core is completely unchanged (obligations, verification, verdicts, enforcement)

### Evidence for review
- `src/core/llm_provider.py` (new provider abstraction)
- Diff of 4 refactored call-site files (zero `genai` references)
- `tests/test_llm_provider.py` results
- Full test suite pass (`pytest tests/`)
- Anthropic-mode benchmark run artifacts:
...
| Anthropic API rate limits or latency | Demo/benchmark runs fail or timeout | Set reasonable `max_tokens`; Anthropic rate limits are generous for single-pipeline usage |
